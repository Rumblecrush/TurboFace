...
-- This library is Classic-only (the project guard above returns on Forever).
-- Never bind Blizzard's private C_CombatLogInternal namespace: it is not an
-- addon contract and cannot be used to reconstruct restricted combat state.
...
local npcSpellTableStarted = false
local function StartNPCSpellTableProcessing()
    if npcSpellTableStarted then return end
    npcSpellTableStarted = true
    lib.NPCSpellTableTimer = C_Timer.NewTimer(0, processNPCSpellTable)
end
-- Preserve consumers that registered against an older embedded copy before
-- this library revision was loaded.
if lib.activeFrames and next(lib.activeFrames) then
    StartNPCSpellTableProcessing()
end
...
-- Embedded users may load the library without actually consuming duration
-- tracking. Keep the maintenance ticker dormant until at least one frame calls
-- RegisterFrame(); UnregisterFrame() tears it back down when the final consumer
-- leaves. This preserves LCD behavior for active consumers without a permanent
-- timer for addons that merely embed the library.
...
function lib:RegisterFrame(frame)
    ...
    if next(activeFrames) then
        if not lib.purgeTicker then
            lib.purgeTicker = C_Timer.NewTicker(PURGE_INTERVAL, purgeOldGUIDs)
        end
        StartNPCSpellTableProcessing()
        if lib.setwatcher and next(itemSets) then
            lib.setwatcher:RegisterUnitEvent("UNIT_INVENTORY_CHANGED", "player")
            if lib.setwatcher.UNIT_INVENTORY_CHANGED then
                lib.setwatcher:UNIT_INVENTORY_CHANGED(nil, "player")
            end
        end
        if CombatLogGetCurrentEventInfo then f:RegisterEvent(COMBAT_LOG_EVENT) end
...
function lib:UnregisterFrame(frame)
    ...
    if not next(activeFrames) then
        ...
        if lib.setwatcher then
            lib.setwatcher:UnregisterEvent("UNIT_INVENTORY_CHANGED")
        end
        if CombatLogGetCurrentEventInfo then f:UnregisterEvent(COMBAT_LOG_EVENT) end
        if playerClass == "ROGUE" then
...
function lib:ToggleDebug()
    ...
    if not lib.debug.enabled then
        if not CombatLogGetCurrentEventInfo then
            print("[LCD] No addon-readable combat log API")
            return
        end
        lib.debug:RegisterEvent(COMBAT_LOG_EVENT)
...
function setwatcher:PLAYER_LOGIN()
    if next(activeFrames) and next(itemSets) then
